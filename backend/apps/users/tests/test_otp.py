...
def test_expired_code_is_refused(api, phone, password, last_sms_code):
    ...
    response = api.post(
        "/api/auth/otp/verify/", {"phone": phone, "code": code, "purpose": "SIGNUP"},
        format="json",
    )
    assert response.status_code == 400
    assert response.data["error"]["code"] == "otp_invalid"
...
def test_max_attempts_locks_the_code(api, phone, password, last_sms_code):
    ...
    for _ in range(settings.OTP_MAX_ATTEMPTS):
        response = api.post(
            "/api/auth/otp/verify/", {"phone": phone, "code": wrong, "purpose": "SIGNUP"},
            format="json",
        )
...
def test_resend_works_after_cooldown_and_invalidates_previous_code(
    ...
    assert new_code != first_code or True  # le code peut être identique : on teste l'invalidation

    assert (
        api.post(
            "/api/auth/otp/verify/", {"phone": phone, "code": first_code, "purpose": "SIGNUP"},
            format="json",
        ).status_code
        == 400
    )
    assert api.post(
        "/api/auth/otp/verify/", {"phone": phone, "code": new_code, "purpose": "SIGNUP"},
        format="json",
    ).status_code == 200
...
def test_otp_purpose_is_strict(api, phone, password, reset_otp, register_and_activate):
    ...
    response = api.post(
        "/api/auth/otp/verify/", {"phone": phone, "code": reset_code, "purpose": "SIGNUP"},
        format="json",
    )
    assert response.status_code == 400
    assert response.data["error"]["code"] == "otp_invalid"
